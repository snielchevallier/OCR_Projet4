<section class="bg-tomTroc-bg ">
                <div class="pt-4 pb-0 max-w-full">
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        ...
                        <div class="">
                            <img src="<?= UPLOADS_URL ?>books/<?= $book->getCover() ?>" class="w-full" alt="couverture du livre <?= $book->getTitle() ?>">
                        </div>
                         <div class="bg-tomTroc-light py-12 px-5 md:ps-16 md:pe-20">
                            ...
                            <h2 class="pt-5 pb-2 font-inter font-semibold text-xs text-tomTroc-darkgrey">PROPRIÉTAIRE</h2>
                                ...
                                <a href="/profil?id=<?=$owner->getId()?>" class="inline-flex  items-center gap-3 px-3 py-1.5 bg-white rounded-full shadow-sm">
                                    <?php if ($owner->getPhoto()): ?>
                                        <img src="<?= UPLOADS_URL ?>users/<?=$owner->getPhoto()?>" class="w-12 h-12 rounded-full object-cover" alt="photo de profil <?=$owner->getPseudo()?>" loading="lazy">
                                    <?php else: ?>
                                        <img src="<?= ASSETS_PATH ?>img/icon-profile-default" class="w-12 h-12 rounded-full object-cover" alt="photo de profil <?=$owner->getPseudo()?>" loading="lazy">
                                    <?php endif; ?>
