            ...
            <section class="bg-tomTroc-bg ">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pb-10">
                         <div class="bg-white py-12 px-5 flex flex-col items-center rounded-lg md:ps-16 md:pe-20">
                            ...
                            <?php if ($user->getPhoto()): ?>
                               <img src="<?= htmlspecialchars(UPLOADS_URL.'users/'.$user->getPhoto(), ENT_QUOTES, 'UTF-8')?>" class="w-32 h-32 rounded-full" alt="photo de profil de <?=htmlspecialchars($user->getPseudo(), ENT_QUOTES, 'UTF-8')?>" loading="lazy">
                            <?php else: ?>
                                <img src="<?= htmlspecialchars(ASSETS_PATH.'img/icon-profile-default', ENT_QUOTES, 'UTF-8')?>" class="w-32 h-32 rounded-full" alt="photo de profil de <?=htmlspecialchars($user->getPseudo(), ENT_QUOTES, 'UTF-8')?>" loading="lazy">
                            <?php endif; ?>
                            <label for="photo" class="pt-2 pb-5 text-tomTroc-grey text-sm font-inter cursor-pointer hover:underline">
                                modifier
                            </label>
                            <hr class="pb-5 px-24">
                            <p class="pb-5 text-tomTroc-darkgrey font-playfairDisplay text-2xl"><?=htmlspecialchars($user->getPseudo(), ENT_QUOTES, 'UTF-8')?></p>
                            ...
                            <form method="POST" action="/updateUser" class="space-y-6" enctype="multipart/form-data">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        name="pseudo"
                                        id="pseudo"
                                        value="<?=htmlspecialchars($user->getPseudo(), ENT_QUOTES, 'UTF-8')?>"
                                        class="w-full px-4 py-3 text-sm border border-tomTroc-lightgrey rounded-lg bg-tomTroc-blue focus:outline-none focus:ring-2 focus:ring-tomTroc-green"
                                    >
                                </div>
                                <button 
                ...
                <div class="mx-auto pt-4 pb-0 px-5 max-w-6xl">
                    <?php if (Utils::request("bookErrorMessage")): ?>
                        <p class="text-sm font-inter text-tomTroc-red text-center my-6 md:my-12">
                            <?=htmlspecialchars(Utils::request("bookErrorMessage"), ENT_QUOTES, 'UTF-8')?>
                        </p>
                        <?php endif; ?>
                        <?php if (Utils::request("bookMessage")): ?>
                        <p class="text-sm font-inter text-tomTroc-green text-center my-6 md:my-12">
                            <?=htmlspecialchars(Utils::request("bookMessage"), ENT_QUOTES, 'UTF-8')?>
                        </p>
                    <?php endif; ?>
                    <div class="gap-5 pb-10 rounded-lg space-y-6 md:space-y-0">
                        ...
                        <div class="bg-white grid <?= $i%2 === 1 ? ' md:bg-tomTroc-blue' : '' ?> grid-cols-2 grid-cols-[90px_1fr] md:grid-cols-[90px_1fr_1fr_2fr_1fr_90px_90px] md:grid-cols-7 p-14 md:ps-16 md:pe-20 md:py-5 justify-items-left items-center rounded-lg md:rounded-none">
                            <div class="order-1 row-span-3">
                                ...
                                <?php else: ?>
                                    <img src="<?= htmlspecialchars(ASSETS_PATH.'img/icon-book-default.png', ENT_QUOTES, 'UTF-8')?>" alt="<?=htmlspecialchars($book->getTitle(), ENT_QUOTES, 'UTF-8')?>" class="w-20 h-20 aspect-square" loading="lazy">
                                <?php endif; ?>
                            </div>
                            <div class="order-2 font-inter text-sm leading-3 md:px-2"><?=htmlspecialchars($book->getTitle(), ENT_QUOTES, 'UTF-8')?></div>
                            <div class="order-3 font-inter text-sm leading-3 md:px-2"><?=htmlspecialchars($book->getAuthor(), ENT_QUOTES, 'UTF-8')?></div>
                            <div class="order-5 col-span-2 font-inter italic text-sm pt-4 md:px-2 md:pt-0 md:order-4 md:col-span-1"><?=htmlspecialchars(substr($book->getDescription(),0,100), ENT_QUOTES, 'UTF-8')?>...</div>
                            ...
                            <div class="order-6 font-inter pt-10 md:pt-0 md:px-2"><a href="/account/editer-livre?id=<?=htmlspecialchars($book->getId(), ENT_QUOTES, 'UTF-8')?>" class="font-inter text-tomTroc-darkgrey text-base">Éditer</a></div>
                            <div class="order-7 font-inter pt-10 md:pt-0 md:px-2"><a href="/supprimer-livre?id=<?=htmlspecialchars($book->getId(), ENT_QUOTES, 'UTF-8')?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce livre ?');" class="font-inter text-base text-tomTroc-red">supprimer</a></div>
                        </div>
                        <?php 
                        $i++; 
                        endforeach; ?>
